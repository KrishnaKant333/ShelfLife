...
export function accumulateExtractions(
  ...
  if (!previous) {
    return {
      ...
      expiryType: next.expiryType ?? null,
      isEstimated: next.isEstimated ?? false,
      daysEstimated: next.daysEstimated,
  ...
  return {
    ...
    expiryType: isPrevExpiryHigh && previous.expiryType ? previous.expiryType : next.expiryType ?? previous.expiryType ?? null,
    isEstimated: isPrevExpiryHigh ? (previous.isEstimated ?? false) : (next.isEstimated ?? previous.isEstimated ?? false),
    daysEstimated: isPrevExpiryHigh ? previous.daysEstimated : next.daysEstimated ?? previous.daysEstimated,
    primaryImageIndex: 0,
